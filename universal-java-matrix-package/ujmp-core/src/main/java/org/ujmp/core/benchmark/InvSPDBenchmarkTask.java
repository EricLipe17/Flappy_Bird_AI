...
public class InvSPDBenchmarkTask extends AbstractBenchmarkTask {
	...
	public InvSPDBenchmarkTask(long benchmarkSeed, Class<? extends DoubleMatrix2D> matrixClass,
	...
	public BenchmarkResult task(Class<? extends Matrix> matrixClass, long benchmarkSeed, int run,
		...
		try {
			...
			if (!m.getClass().getName().startsWith("org.ujmp.core")
					&& m.getClass().getDeclaredMethod("invSPD") == null) {
				System.out.print("-");
				System.out.flush();
			...
			double diff = BenchmarkUtil.difference(result,
					DenseDoubleMatrix2D.Factory.eye(m.getRowCount(), m.getColumnCount()));
			result = null;
